...
using namespace std;
void expDrawFunction(char * text, int x, int y) {
	cout << "expDrawFunction called." << endl;
	fl_color(TICK_COLOR);
...
class MyTimer : public Fl_Box {
	...
	//	static long start = time(NULL);
		...
		expDrawFunction(secs, xToPass, yToPass);
...
int main() {
	...
	std::thread threadOne(ercoClockRun);

//	std::thread threadTwo(ercoClockRun);
